...
- (NSString *)convertVccDateStringToInternationalFormat:
  ...
  if (![self checkIfVccDateTimeStringIncludesTimezone:vccDateString]) {
    stringGuaranteedToHaveTimezone =
        [self appendZeroTimezoneToVccDateString:vccDateString];
  } else {
  ...
  NSString *stringWithSpaceInFrontOfTimezone =
      [self putSpaceBetweenTimeAndTimezone:
                stringWithSpaceInsteadOfTBetweenDateAndTime];

  NSString *stringWithoutColon =
      [self removeTimezoneColonFromDateString:stringWithSpaceInFrontOfTimezone];

  return stringWithoutColon;
}

- (NSString *)appendZeroTimezoneToVccDateString:(NSString *)vccDateString {
  NSString *stringWithZeroTimezoneAppended =
      [vccDateString stringByAppendingString:@"+00:00"];
